    ...
    function ngGallery($document, $timeout, $q, $templateCache) {
        ...
        $templateCache.put(template_url,
            '<div class="{{ baseClass }}">' +
            '   <img ng-src="{{ images[0].img }}" ng-click="openGallery(0)" alt="{{title}} - {{ $index + 1 }}" title="{{title}}" />' +
            '</div>' +
            ...
            '  <a href="{{getImageDownloadSrc()}}" target="_blank" ng-show="showImageDownloadButton()" class="download-image"><md-icon md-font-icon="icon-download" class="s32 md-background-fg"></md-icon></a>' +
            '  <a class="close-popup" ng-click="closeGallery()"><md-icon md-font-icon="icon-close" class="s48 md-background-fg"></md-icon></a>' +
            '  <a class="nav-left" ng-click="prevImage()"><md-icon md-font-icon="icon-chevron-left" class="s48 md-background-fg"></md-icon></a>' +
            '  <img ondragstart="return false;" draggable="false" ng-src="{{ img }}" ng-click="nextImage()" ng-show="!loading" class="effect" alt="{{title}} - {{ $index + 1 }}"  title="{{title}} - {{ $index + 1 }}"/>' +
            '  <a class="nav-right" ng-click="nextImage()"><md-icon md-font-icon="icon-chevron-right" class="s48 md-background-fg"></md-icon></a>' +
            '  <span class="info-text">{{title}} - {{ index + 1 }}/{{ images.length }}</span>' +
            '  <div class="ng-thumbnails-wrapper">' +
            '    <div class="ng-thumbnails slide-left">' +
            '      <div ng-repeat="i in images">' +
            '        <img ng-src="{{ i.thumb }}" ng-class="{\'active\': index === $index}" ng-click="changeImage($index)" alt="{{title}} - {{ $index + 1 }}" title="{{title}} - {{ $index + 1 }}" />' +
            '      </div>' +
            '    </div>' +
            '  </div>' +
            '</div>'
